...
// Rotas admin específicas (montar antes do admin genérico)
...
// aliases para compat com o front
...
app.use("/api/config", configPublicRouter);      // pública (front lê o preço)
app.use("/api/admin/config", adminConfigRouter); // admin (salva o preço)

// ── router admin GENÉRICO (por último) ─────────────────────
...
async function bootstrap() {
  try {
    await ensureSchema();
    await ensureAppConfig(); // garante a chave 'ticket_price_cents'
