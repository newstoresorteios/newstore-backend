 ...
 * Idempotente: usa delta de pagamentos calculado no servidor.
 * add_cents (cliente) é apenas um "cap" para não ultrapassar.
 */
router.post("/sync", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const addRaw = req.body?.add_cents ?? req.body?.addCents ?? 0;
    let addCents = Number.isFinite(Number(addRaw)) ? parseInt(addRaw, 10) : 0;
    if (addCents < 0) addCents = 0;
    ...
    // 2) Calcula delta real no servidor
    const { delta_cents, max_t } = await computeServerDelta(uid, cur.last_payment_sync_at);
    // Cap pelo que o cliente indicou (segurança; evita creditar mais do que ele detectou)
    const applyCents = Math.max(0, Math.min(delta_cents, addCents));

    console.log(
      `[coupons.sync#${rid}] user=${uid} addCents=${addCents} serverDelta=${delta_cents} willApply=${applyCents} lastSync=${cur.last_payment_sync_at || null} maxT=${max_t || null}`
    );
    ...
    if (applyCents > 0 && max_t) {
      ...
      if (upd.rowCount > 0) {
        ...
        // outro request aplicou antes; recarrega
