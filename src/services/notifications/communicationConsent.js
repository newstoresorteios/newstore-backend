...
function normalizeCategory(category) {
  ...
  if (!raw) return WHATSAPP_CONSENT_CATEGORY_DEFAULT;
  return ALLOWED_CATEGORIES.has(raw) ? raw : WHATSAPP_CONSENT_CATEGORY_DEFAULT;
}
...
export function normalizeWhatsAppConsentCategory(category) {
  return normalizeCategory(category);
}

export function isAllowedWhatsAppConsentCategory(category) {
  return ALLOWED_CATEGORIES.has(cleanText(category).toLowerCase());
}
...
export async function getUserCommunicationConsent({
  ...
  try {
    const categories = safeCategory === "all" ? ["all"] : [safeCategory, "all"];
    const result = await runQuery(
      ...
      `SELECT user_id, channel, category, status, source, created_at
        ...
        WHERE user_id = $1
          ...
          AND LOWER(category) = ANY($3::text[])
        ORDER BY created_at DESC
        LIMIT 1`,
      [uid, safeChannel, categories]
    );
    ...
    return {
      user_id: uid,
      channel: safeChannel,
      category: row?.category || safeCategory,
...
export async function canSendWhatsAppToUser({
  ...
  const consent = await getWhatsappConsentStatusForUser({ pgClient, userId, category });
  return consent.whatsapp_can_send === true;
}
    ...
    const result = await runQuery(
      ...
      `WITH latest_consent AS (
         SELECT DISTINCT ON (user_id)
                user_id, LOWER(status) AS status, LOWER(category) AS category
           FROM public.communication_consents
...
export async function createWhatsAppCommunicationConsent({
  pgClient,
  userId,
  category = "all",
  status,
  ...
  try {
    const result = await runQuery(
      ...
      `INSERT INTO public.communication_consents
         (user_id, channel, category, status, source, ip, user_agent, meta, created_at)
       VALUES ($1, 'whatsapp', $2, $3, $4, $5, $6, $7::jsonb, NOW())
       RETURNING user_id, channel, category, status, source, created_at`,
      [
        uid,
        safeCategory,
        safeStatus,
        cleanText(source, "account_page") || "account_page",
        ip ? String(ip).slice(0, 128) : null,
        userAgent ? String(userAgent).slice(0, 512) : null,
        JSON.stringify(meta && typeof meta === "object" && !Array.isArray(meta) ? meta : {}),
      ]
    );
    return { ok: true, consent: result.rows?.[0] || null };
  } catch (error) {
    if (error?.code === "42P01" || error?.code === "42703") {
      console.warn("[whatsapp-consent] write:blocked", {
        user_id: uid,
        category: safeCategory,
        schema_error: true,
      });
      return { ok: false, error: "whatsapp_consent_unknown" };
    }
    throw error;
  }
}
